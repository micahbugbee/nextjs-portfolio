...
function ContactMe({}: Props) {
    const {
      ...
      const onSubmit: SubmitHandler<Inputs> = (formData) => {
        window.location.href = `mailto:micahbugbee@hotmail.com?subject=${formData.subject}&body=Hi, my name is ${formData.name}. ${formData.message} (${formData.email})`;
      };
    ...
    <div className='h-screen flex relative flex-col text-center md:text-left md:flex-row max-w-7xl px-10 justify-evenly items-center mx-auto'>
        ...
        <div className='flex flex-col space-y-10'>
            ...
            <div className='space-y-10'>
                ...
                <p className='text-2xl'>580-221-0585</p>
                </div>

                <div className='flex items-center space-x-5 justify-center'>
                <EnvelopeIcon className='text-[#F7AB0A] h-7 w-7 animate-pulse' />
                <p className='text-2xl'>micahbugbee@hotmail.com</p>
                </div>

                <div className='flex items-center space-x-5 justify-center'>
                <MapPinIcon className='text-[#F7AB0A] h-7 w-7 animate-pulse' />
                <p className='text-2xl'>9101 NW 119th St</p>
                </div>
            </div>
